...
- **Moderate**: Long conversation or several large files loaded → consider `/compact` soon
- **Heavy**: Very long conversation or context feels sluggish → run `/compact` before next task
...
> Run `/compact` before starting the next task to keep things clean.
